...
/**
 * Background NTP sync task (runs once, then deletes itself)
 *
 * This task runs asynchronously on Core 1 to avoid blocking startup.
 * It connects to WiFi, syncs TimeManager via NTP, then disconnects and exits.
 *
 * Runs as FreeRTOS task - does not block main initialization.
 * Remove when Phase 3 WiFi/MQTT is implemented.
 */
void backgroundNTPSyncTask(void* parameter) {
    // Wait 2 seconds for system to stabilize
    vTaskDelay(pdMS_TO_TICKS(2000));

    Serial.println("\n[Background NTP] Starting async NTP sync...");

    if (!g_networkConfig) {
        Serial.println("[Background NTP] Skipping - NetworkConfig not available");
    ...
    if (strlen(wifi_settings.ssid) == 0) {
        Serial.println("[Background NTP] ERROR: No WiFi SSID in network.ini");
        vTaskDelete(NULL);
        return;
    }
    ...
    if (WiFi.status() != WL_CONNECTED) {
        Serial.println("[Background NTP] WiFi connection failed");
        WiFi.disconnect(true);
        vTaskDelete(NULL);
        return;
    }

    Serial.printf("[Background NTP] WiFi connected, IP: %s\n", WiFi.localIP().toString().c_str());

    // Sync TimeManager with NTP (TimeManager already created in setup())
    if (g_timeManager && g_timeManager->syncNow()) {
        Serial.println("[Background NTP] NTP sync successful!");
        ...
        Serial.printf("[Background NTP] Time: %s %s (synced from %s)\n",
                      date_str, time_str, ntp_settings.server);
    } else {
        Serial.println("[Background NTP] NTP sync failed");
    }

    // Disconnect WiFi
    WiFi.disconnect(true);
    Serial.println("[Background NTP] Complete, WiFi disconnected\n");

    // Delete this task (runs once only)
    vTaskDelete(NULL);
}
...
void setup() {
    ...
    if (!g_timeManager->begin(timezone_offset, g_sdManager)) {
        ...
        g_timeManager->getTimeString(time_str, sizeof(time_str));
        g_timeManager->getDateString(date_str, sizeof(date_str));
        Serial.printf("[TimeManager] Current time: %s %s\n", date_str, time_str);
    }
    ...
    if (g_networkConfig) {
        BaseType_t ntp_result = xTaskCreatePinnedToCore(
            ...
            8192,                  // Stack size (8KB)
            NULL,                  // Parameters
            1,                     // Priority (same as others)
            NULL,                  // No handle needed (task deletes itself)
            1                      // Core 1 (same as network)
        );

        if (ntp_result == pdPASS) {
            Serial.println("[OK] Background NTP sync task created (will run after 2 sec delay)");
        } else {
